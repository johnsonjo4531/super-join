...
/// SQL aggregate function applied to one term (or `*` for `Count` with no term).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub enum Expression {
    ...
    /// SQL aggregate over one term (`*` when `term` is `None`, only meaningful
    /// for `Count`). Aggregates appear in computed-field select expressions.
    Aggregate {
        function: AggregateFunction,
        term: Option<Box<Expression>>,
    },
}
