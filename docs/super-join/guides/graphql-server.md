...
Super-Join is a compiler only. It does not own a connection, a pool, or execution; you bring any SQL driver you like. See [result-shape-and-hydration.md](result-shape-and-hydration.md) for what the artifact contains and how to regroup its rows.
...
- A relation's `join` is a flattened expression (see [filtering-and-hooks.md](filtering-and-hooks.md)). `parent-column` refers to the parent entity occurrence; `column` refers to the target entity.
...
- `dataType` must be one of the compiler's scalar types. The current enum has no `text`/`varchar` variant yet (see [Current limitations](#current-limitations)), so this guide models numeric columns only.
...
export const superJoinModel: GraphQLModel = {
  ...
  fieldForEntity(entityId, fieldName) {
    if (entityId === USER_ENTITY_ID) {
      return fieldName === "id" ? USER_ID : undefined;
    }
    switch (fieldName) {
...
async function queryUsers(_args, context, info) {
  ...
  return hydrate(rows, artifact); // see result-shape-and-hydration.md
}
```
...
The full code table lives in [filtering-and-hooks.md](filtering-and-hooks.md#the-error-code-table).

## Current limitations

Super-Join is an alpha prototype. At the time of writing:

- The scalar-type enum has no `text`/`varchar` variant, so string columns cannot be selected yet; model them as `selectable: false` until a text type lands.
...
- Arguments never become SQL filters unless the field opts in via `fields[name].filterArgs` (see [filtering-and-hooks.md](filtering-and-hooks.md#field-level-argument-options)).
- The GraphQL frontend expects the GraphQL.js v16 shape of `variableValues`; on GraphQL.js v17 pass the coerced map (see [filtering-and-hooks.md](filtering-and-hooks.md#variables)).

## Next steps

- [filtering-and-hooks.md](filtering-and-hooks.md) — arguments as filters, pagination, ordering, and hooks.
