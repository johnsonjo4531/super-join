...
const generated = graphQLModelFromClasses<Context>([User, Post], { dialect: 'sqlite' });
...
const execute = (artifact: SqlArtifact): Record<string, unknown>[] => {
  const statement = db.prepare(artifact.sql);
  return statement.all(...artifact.parameters.map(toDriverValue)) as Record<string, unknown>[];
};

async function queryUsers(_args: Record<string, unknown>, context: Context, info: GraphQLResolveInfo) {
  return superjoin.graphql({ resolveInfo: info, context, model, execute });
}

async function queryUser(_args: Record<string, unknown>, context: Context, info: GraphQLResolveInfo) {
...
async function readJsonBody(request: import('node:http').IncomingMessage): Promise<{ query?: string; variables?: Record<string, unknown> }> {
  const chunks = [];
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    response.end(JSON.stringify({ errors: [{ message: `${(error as Error).message}${code}` }] }));
  }
});
