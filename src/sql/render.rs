...
use crate::expression::{BooleanOperator, ComparisonOperator, IsNullOperator, Parameter, Value};
use crate::model::ScalarType;
use crate::relational::{ColumnRef, JoinType, OrderBySql, RelExpr};
...
fn render_column(col: &SqlColumn, dialect: &Dialect) -> String {
    format!(
        "{} AS {}",
        render_column_ref(&col.source, dialect),
        quote_ident(&col.alias, dialect)
    )
}
