...
//!
//! The WIT boundary cannot express recursive types, so the `expression` tree and
//! the query/selection graph are serialized as flat lists of nodes with
...
use crate::expression::{
    BooleanOperator, ComparisonOperator, Expression, IsNullOperator, Parameter, Value,
};
use crate::model::{
    Cardinality, EntityMetadata, FieldMetadata, Identifier, Model, RelationMetadata, ScalarType,
};
use crate::semantic::{OrderDirection, OrderBy, QueryNode, Selection};
use crate::sql::{Dialect, Param, SqlArtifact};

use crate::wit::exports::super_join::compiler::compiler as wit;
use wit::{
    ...
    ScalarType as WitScalarType, SelectionKind as WitSelectionKind, SelectionNode as WitSelectionNode,
    SqlArtifact as WitSqlArtifact, SqlDialect as WitSqlDialect, SortDirection as WitSortDirection,
    Value as WitValue,
};
...
fn convert_field(f: &wit::FieldMetadata) -> FieldMetadata {
    FieldMetadata {
        id: f.id,
...
fn convert_relation(
    r: &wit::RelationMetadata,
) -> Result<RelationMetadata, CompilerError> {
    Ok(RelationMetadata {
        id: r.id,
        target: r.target,
...
fn convert_entity(
    e: &wit::EntityMetadata,
) -> Result<EntityMetadata, CompilerError> {
...
fn convert_request(r: WitCompilerRequest) -> Result<crate::CompilerRequest, CompilerError> {
...
fn convert_selected_field(f: &crate::sql::SqlColumn) -> wit::SelectedField {
    wit::SelectedField {
        alias: f.alias.clone(),
        field: f.field_id,
        path: f.path.clone(),
...
fn convert_result_shape(
    kind: crate::sql::ResultShapeKind,
) -> wit::ResultShapeKind {
    match kind {
        crate::sql::ResultShapeKind::Flat => wit::ResultShapeKind::Flat,
        crate::sql::ResultShapeKind::Nested => wit::ResultShapeKind::Nested,
        crate::sql::ResultShapeKind::Json => wit::ResultShapeKind::Json,
    }
}

fn convert_error(e: &crate::error::CompilerError) -> WitCompilerError {
    WitCompilerError {
...
fn convert_source(loc: &crate::error::SourceLocation) -> wit::SourceLocation {
    wit::SourceLocation {
        path: loc.path.clone(),
        line: loc.line,
        column: loc.column,
...
impl wit::Guest for Component {
    fn compile(request: WitCompilerRequest) -> Result<WitCompilerResult, WitCompilerError> {
        ...
        let compiler = crate::compiler::Compiler::new(core_request.dialect);
